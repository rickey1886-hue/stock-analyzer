...
if run:
    ...
    def get_horizon_view(horizon_label: str) -> str:
        horizon_data = horizons.get(horizon_label, {})
        if isinstance(horizon_data, dict):
            return horizon_data.get("view", "データ未取得")
        return "データ未取得"
    ...
    cards = [
        ...
        ("短期判定", f"<span class='{judgment_class(get_horizon_view('短期'))}'>{get_horizon_view('短期')}</span>"),
        ("中期判定", f"<span class='{judgment_class(get_horizon_view('中期'))}'>{get_horizon_view('中期')}</span>"),
        ("長期判定", f"<span class='{judgment_class(get_horizon_view('長期'))}'>{get_horizon_view('長期')}</span>"),
    ]
    ...
    with tab_summary:
        ...
        horizon_rows = []
        for term in ["短期", "中期", "長期"]:
            data = horizons.get(term, {})
            buy_list = data.get("buy", []) if isinstance(data, dict) else []
            caution_list = data.get("caution", []) if isinstance(data, dict) else []
            buy_text = "、".join(buy_list) if buy_list else "データ未取得"
            caution_text = "、".join(caution_list) if caution_list else "データ未取得"
            horizon_rows.append({"期間": term, "判定": get_horizon_view(term), "買い材料": buy_text, "売り材料 / 注意材料": caution_text})
