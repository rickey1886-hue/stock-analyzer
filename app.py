...
if run:
    ...
    latest_close = float(close_series.iloc[-1]) if not close_series.empty else stats.get("latest_close")
    ...
    def resolve_view_text(horizon_data: dict) -> str:
        view = horizon_data.get("view", "データ未取得")
        if view != "データ未取得":
            return view
        buy_items = horizon_data.get("buy", [])
        caution_items = horizon_data.get("caution", [])
        if buy_items or caution_items:
            if buy_items and caution_items:
                return "強弱材料が混在"
            if buy_items:
                return "やや強含み"
            return "注意シグナルあり"
    ...
    short_view = resolve_view_text(short_horizon)
    mid_view = resolve_view_text(mid_horizon)
    long_view = resolve_view_text(long_horizon)
    ...
    with tab_summary:
        ...
        for term, data in display_horizons.items():
            ...
            view = resolve_view_text(data) if isinstance(data, dict) else "データ未取得"
