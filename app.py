...
if run:
    ...
    horizon_map = {item["期間"]: item["判定"] for item in [{"期間": term, "判定": data["view"]} for term, data in horizons.items()]}
    top_cols = st.columns(6)
    top_cols[0].metric("総合判定", judgment)
    top_cols[1].metric("総合スコア", f"{score} / 100")
    top_cols[2].metric("直近終値", format_number(stats["latest_close"], 2))
    top_cols[3].metric("短期", horizon_map.get("短期", "データ未取得"))
    top_cols[4].metric("中期", horizon_map.get("中期", "データ未取得"))
    top_cols[5].metric("長期", horizon_map.get("長期", "データ未取得"))

    tabs = st.tabs(["サマリー", "チャート", "テクニカル", "ファンダメンタル", "市場環境", "判定理由・リスク"])
    ...
    with tabs[3]:
        ...
        st.dataframe(pd.DataFrame(fundamental_rows.items(), columns=["項目", "値"]), use_container_width=True, hide_index=True)
    ...
    with tabs[5]:
        ...
        col_buy, col_caution = st.columns(2)
        with col_buy:
            st.markdown("**買い材料**")
            if buy_factors:
        ...
        if missing:
            st.caption("以下の項目は今回の取得対象で欠損がありました。")
            for item in missing:
                st.markdown(f"- {item}")
        else:
            st.caption("データ未取得項目はありません。")
